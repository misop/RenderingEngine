...
void AssimpObject::LoadTexturesFromFile(string fileName) {
	...
	while (!inputFile.eof()) {
		...
		case 'a':
			inputFile >> str1;
			diffuseTexture = shared_ptr<GL::Texture>(new GL::Texture(GL_TEXTURE_2D));
			diffuseTexture->LoadRGBATextureFromImage(str1);
			break;
		case 'd':
			inputFile >> str1;
			displacementTexture = shared_ptr<GL::Texture>(new GL::Texture(GL_TEXTURE_2D));
			displacementTexture->LoadRGBATextureFromImage(str1);
			break;
		case 'n':
			inputFile >> str1;
			normalTexture = shared_ptr<GL::Texture>(new GL::Texture(GL_TEXTURE_2D));
