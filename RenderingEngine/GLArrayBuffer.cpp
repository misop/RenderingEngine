...
void ArrayBuffer::Draw(GLenum mode, bool bind) {
...
void ArrayBuffer::DrawElement(int element, GLenum mode, bool bind) {
	if (vertices == 0) return;
...
void ArrayBuffer::BindBufferDataf(std::vector<float> &data, GLint elements, GLenum storageMode, GLboolean normalize, GLsizei stride, GLvoid* offset, bool bind) {
...
void ArrayBuffer::BindBufferDatai(std::vector<int> &data, GLint elements, GLenum storageMode, GLsizei stride, GLvoid* offset, bool bind) {
	if (data.size() == 0) return;
	if (bind) glBindVertexArray(vao);
...
void ArrayBuffer::Attrib3f(float a, float b, float c, bool bind) {
	if (bind) glBindVertexArray(vao);
...
int ArrayBuffer::BindElement(std::vector<int> &data, GLenum storageMode) {
